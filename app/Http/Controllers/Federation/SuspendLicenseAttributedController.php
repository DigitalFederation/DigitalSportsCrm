...
class SuspendLicenseAttributedController extends Controller
{
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($user) {
                $sendNotification = new RevokeUserRolesForSuspendedLicenseAction;
                $sendNotification->execute($user, $license);
            }
        } catch (\Exception $e) {
        ...
        // redirect back
        return redirect(route('federation.license-attributed.show', $license->id))->with('success', 'License suspended with success.');
    }
}
