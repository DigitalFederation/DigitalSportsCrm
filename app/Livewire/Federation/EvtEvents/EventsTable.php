...
namespace App\Livewire\Federation\EvtEvents;
...
class EventsTable extends Component implements HasForms, HasTable
{
        ...
        return $table
            ...
            ->actions([
                Action::make('enter')
                    ...
                    ->url(fn (Event $record): string => route('federation.evt-events.events.show', $record->id))
                    ->button()
                    ->color('primary'),
            ])
            ->bulkActions([])
    ...
    {
        $query = Event::query()->with('competition.sport');

        if (! $this->isDefaultFederation()) {
            $currentCountryId = Auth::user()->federations()->first()->country_id;

            $query->where('is_visible', true)
                ->where(function ($q) use ($currentCountryId) {
                    $q->where('event_geographical_coverage', 'international')
                        ->orWhere(function ($sub) use ($currentCountryId) {
                            $sub->where('event_geographical_coverage', 'national')
                                ->whereHas('countries', function ($subQuery) use ($currentCountryId) {
                                    $subQuery->where('country.id', $currentCountryId);
                                });
                        })
                        ->orWhereDoesntHave('geographicLimitations');
                })
                ->whereNotIn('enrollment_type', [
    ...
    {
        return view('livewire.federation.evt-events.events-table');
    }
}
