<div>
    <x-information-box
        :title="__('reports.insurance_reports_title')"
        :body="__('reports.insurance_report_description')"
    />
        ...
        <div class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div>
                    ...
                    >
                        <option value="individual">{{ __('main.individual_type') }}</option>
                        <option value="entity">{{ __('main.entity_type') }}</option>
                    </select>
                </div>
                ...
                <tr wire:key="report-{{ $report->id }}">
                    ...
                    <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                        <div class="flex items-center">
                            ...
                            @else
                                <span class="text-gray-500">{{ __('reports.system_generated') }}</span>
                            @endif
                        </div>
                    </td>
                    <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                    ...
                    <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                        @if($report->status === 'ready')
                            <select
                                wire:change="updateInsurerStatus({{ $report->id }}, $event.target.value)"
                                class="text-sm rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500
                                    @if($report->insurer_status === 'completed') bg-green-50 text-green-800
                                    @elseif($report->insurer_status === 'sent') bg-blue-50 text-blue-800
                                    @else bg-yellow-50 text-yellow-800
                                    @endif"
                            >
                                <option value="pending" @if($report->insurer_status === 'pending' || !$report->insurer_status) selected @endif>
                                    {{ __('reports.insurer_statuses.pending') }}
                                </option>
                                <option value="sent" @if($report->insurer_status === 'sent') selected @endif>
                                    {{ __('reports.insurer_statuses.sent') }}
                                </option>
                                <option value="completed" @if($report->insurer_status === 'completed') selected @endif>
                                    {{ __('reports.insurer_statuses.completed') }}
                                </option>
                            </select>
                        @else
                            <span class="text-gray-400">-</span>
                    ...
                    <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap text-right">
                        <div class="space-x-1 flex items-center justify-end">
                            ...
                            <button
                                ...
                                wire:confirm="{{ __('reports.confirm_delete_report') }}"
