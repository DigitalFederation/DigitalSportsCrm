<div wire:key="diving-professionals-{{ $this->getId() }}" class="min-h-screen bg-gray-50">
    <div class="flex flex-col md:flex-row" x-data="{ isSidebarOpen: window.innerWidth >= 768 }"
        ...
        <div x-show="isSidebarOpen" x-transition:enter="transition ease-out duration-300"
            ...
            <div class="px-6 py-4 flex-shrink-0 border-b border-gray-200 bg-white">
                <div class="flex items-center justify-between">
                    <div>
                        <h2 class="text-lg font-bold text-gray-900">{{ __('public.diving_professionals.title') }}</h2>
                        <p class="text-xs text-gray-500 mt-1">{{ __('public.diving_professionals.description') }}</p>
                    </div>
            ...
            <div class="px-6 py-4 bg-white flex-1 overflow-y-auto">
                    ...
                    <div class="flex-1 bg-white rounded-xl overflow-hidden shadow-sm border border-gray-100">
                        <div class="px-4 py-3 bg-gradient-to-r from-blue-50 to-blue-100 border-b border-blue-100">
                            <p class="text-xs font-medium text-blue-800 uppercase tracking-wider">
                                {{ __('public.diving_professionals.results') }}
                            </p>
                        </div>
                        <div class="px-4 py-3 flex justify-between items-center">
                            <p class="text-2xl font-bold text-gray-800">{{ $this->professionals->total() }}</p>
                            <span class="text-sm text-gray-500">{{ __('public.diving_professionals.professionals') }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-6">
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <button type="button" wire:click="clearFilters"
                            ...
                            {{ __('public.diving_professionals.clear_filters') }}
                        </button>
                    </div>
                    ...
                    <div class="space-y-4" x-data="{ nameOpen: true, districtOpen: false, statusOpen: false }">
                        ...
                        <div class="bg-white rounded-xl overflow-hidden shadow-sm border border-gray-100">
                            <button type="button" @click="nameOpen = !nameOpen"
                                ...
                                <p class="text-xs font-medium text-blue-800 uppercase tracking-wider">
                                    {{ __('public.diving_professionals.filters.name') }}
                                </p>
                            ...
                            <div x-show="nameOpen" x-collapse class="p-3">
                                <div class="relative">
                                    ...
                                    <input type="text" wire:model.live.debounce.300ms="searchName"
                                        placeholder="{{ __('public.diving_professionals.filters.name_placeholder') }}"
                        ...
                        @if ($this->districts->isNotEmpty())
                            <div class="bg-white rounded-xl overflow-hidden shadow-sm border border-gray-100">
                                <button type="button" @click="districtOpen = !districtOpen"
                                    class="w-full px-4 py-3 bg-gradient-to-r from-blue-50 to-blue-100 border-b border-blue-100 flex justify-between items-center text-left">
                                    <p class="text-xs font-medium text-blue-800 uppercase tracking-wider">
                                        {{ __('public.diving_professionals.filters.district') }}
                                    </p>
                                    <svg class="w-4 h-4 text-blue-600 transform transition-transform duration-200"
                                ...
                                <div x-show="districtOpen" x-collapse class="p-3">
                                    <select wire:model.live="selectedDistrict"
                                        ...
                                        <option value="">{{ __('public.diving_professionals.filters.all_districts') }}</option>
                        ...
                        <div class="bg-white rounded-xl overflow-hidden shadow-sm border border-gray-100">
                            <button type="button" @click="statusOpen = !statusOpen"
                                class="w-full px-4 py-3 bg-gradient-to-r from-blue-50 to-blue-100 border-b border-blue-100 flex justify-between items-center text-left">
                                <p class="text-xs font-medium text-blue-800 uppercase tracking-wider">
                                    {{ __('public.diving_professionals.filters.status') }}
                                </p>
                                <svg class="w-4 h-4 text-blue-600 transform transition-transform duration-200"
                            ...
                            <div x-show="statusOpen" x-collapse class="p-3">
                                <select wire:model.live="selectedStatus"
                                    class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                                    <option value="">{{ __('public.diving_professionals.filters.all_statuses') }}</option>
                ...
                <div class="mt-6 pt-6 border-t border-gray-200">
                    <h3 class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3">
                        {{ __('public.diving_professionals.legend.title') }}
                    </h3>

                    {{-- RNPM Description --}}
                    <div class="mb-4 p-4 bg-blue-50 rounded-xl border border-blue-100">
                        <p class="text-xs text-gray-600 leading-relaxed mb-2">
                            {{ __('public.diving_professionals.subtitle_paragraph_1') }}
                        </p>
                        <p class="text-xs text-gray-600 leading-relaxed">
                            {{ __('public.diving_professionals.subtitle_paragraph_2') }}
                        </p>
                    </div>
                    ...
                    <h4 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">
                        {{ __('public.diving_professionals.legend.subtitle') }}
                    </h4>
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex items-start space-x-2">
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 flex-shrink-0 mt-0.5">
                                {{ __('public.diving_professionals.status.active') }}
                            </span>
                        </div>
                        <p class="text-xs text-gray-600 leading-relaxed">
                            {{ __('public.diving_professionals.legend.active_description') }}
                        </p>
                        ...
                        <div class="flex items-start space-x-2 mt-3">
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-800 flex-shrink-0 mt-0.5">
                                {{ __('public.diving_professionals.status.expired') }}
                            </span>
                        </div>
                        <p class="text-xs text-gray-600 leading-relaxed">
                            {{ __('public.diving_professionals.legend.expired_description') }}
                        </p>
                        ...
                        <div class="flex items-start space-x-2 mt-3">
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800 flex-shrink-0 mt-0.5">
                                {{ __('public.diving_professionals.status.suspended') }}
                            </span>
                        </div>
                        <p class="text-xs text-gray-600 leading-relaxed">
                            {{ __('public.diving_professionals.legend.suspended_description') }}
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div class="flex-1 min-h-screen">
            ...
            <div class="md:hidden sticky top-16 z-20 bg-white border-b border-gray-200 px-4 py-3">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-lg font-bold text-gray-900">{{ __('public.diving_professionals.title') }}</h1>
                        <p class="text-sm text-gray-500">{{ $this->professionals->total() }} {{ __('public.diving_professionals.professionals') }}</p>
                    </div>
                                ...
                                <thead class="bg-gray-50">
                                    <tr>
                                        <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                            {{ __('public.diving_professionals.table.photo') }}
                                        </th>
                                        <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                            {{ __('public.diving_professionals.table.name') }}
                                        </th>
                                        <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                            {{ __('public.diving_professionals.table.district') }}
                                        </th>
                                        <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                            {{ __('public.diving_professionals.table.license') }}
                                        </th>
                                        <th scope="col" class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">
                                            {{ __('public.diving_professionals.table.view_profile') }}
                                        </th>
                                    </tr>
                                </thead>
                                    ...
                                    @foreach ($this->professionals as $professional)
                                        <tr wire:key="professional-{{ $professional->id }}" class="hover:bg-gray-50 transition-colors duration-150">
                                            ...
                                            <td class="px-4 py-3 whitespace-nowrap text-right">
                                                <a href="{{ route('public.diving-professional-profile', $professional) }}"
                ...
                @else
                    ...
                    <div class="text-center py-12">
                        ...
                        <h3 class="text-lg font-medium text-gray-900 mb-1">
                            {{ __('public.diving_professionals.no_results') }}
                        </h3>
                        <p class="text-sm text-gray-500 max-w-md mx-auto mb-4">
                            {{ __('public.diving_professionals.no_results_hint') }}
                        </p>
