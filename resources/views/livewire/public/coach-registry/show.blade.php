<div class="min-h-screen bg-gray-50">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="mb-4">
                <a href="{{ route('public.coach-registry') }}"
                    ...
                    {{ __('public.coach_registry.profile.back') }}
                </a>
            </div>
    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-12 space-y-8">
        ...
        @if ($this->certifications->isNotEmpty())
            <div>
                <h2 class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4 flex items-center gap-2">
                    ...
                    {{ __('public.coach_registry.profile.certifications') }}
                </h2>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-2/5">
                                        {{ __('public.coach_registry.profile.table.certification') }}
                                    </th>
                                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                        {{ __('public.coach_registry.profile.table.expiration_date') }}
                                    </th>
                                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                        {{ __('public.coach_registry.profile.table.certification_status') }}
                                    </th>
                                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                        {{ __('public.coach_registry.profile.table.license_status') }}
                                    </th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($this->certifications as $cert)
                                    @php
                                        $certStatus = $this->getCertificationStatus($cert);
                                        ...
                                        $matchingLicense = $certLicenseId
                                            ...
                                            : null;
                                        $licenseStatus = $matchingLicense ? $this->getLicenseStatusForAttributed($matchingLicense) : null;
                                    @endphp
                                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                                        ...
                                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-600">
                                            {{ $cert->current_term_ends_at?->format('d/m/Y') ?? '-' }}
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap">
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                                {{ $certStatus === 'active' ? 'bg-green-100 text-green-800' : '' }}
                                                {{ $certStatus === 'expired' ? 'bg-amber-100 text-amber-800' : '' }}
                                                {{ $certStatus === 'suspended' ? 'bg-red-100 text-red-800' : '' }}">
                                                {{ __('public.coach_registry.certification_status_values.' . $certStatus) }}
                                            </span>
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap">
                                            @if ($licenseStatus)
        ...
        <div>
            <h2 class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4 flex items-center gap-2">
                <span class="inline-block w-2 h-2 rounded-full bg-blue-600"></span>
                {{ __('public.coach_registry.profile.competitions') }}
            </h2>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                        {{ __('public.coach_registry.profile.sport') }}
                                    </th>
                                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                        {{ __('public.coach_registry.profile.start_date') }}
                                    </th>
                                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                        {{ __('public.coach_registry.profile.end_date') }}
                                    </th>
                                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                        {{ __('public.coach_registry.profile.event') }}
                                    </th>
                                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                        {{ __('public.coach_registry.profile.entity') }}
                                    </th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-200">
            ...
            @else
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-8 text-center">
                    ...
                    <p class="text-sm text-gray-500">{{ __('public.coach_registry.profile.no_competitions') }}</p>
                </div>
            @endif
        </div>
    </div>
