            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div class="flex-1">
                    <div class="flex items-center gap-3 mb-3">
                        <div class="flex items-center justify-center w-10 h-10 rounded-full bg-amber-100">
                            <x-heroicon-s-academic-cap class="w-6 h-6 text-amber-600" />
                        </div>
                    ...
                    <h1 class="text-xl sm:text-2xl font-semibold text-gray-900">{{ __('events.coach_history_title') }}</h1>
                    <p class="mt-1 text-gray-500 text-sm">{{ __('events.coach_history_description') }}</p>
                </div>
                ...
                <div class="flex-shrink-0">
                    <div class="inline-flex flex-col items-center justify-center px-5 py-3 rounded-xl bg-amber-50 border border-amber-200">
                        ...
                        <span class="text-xs font-medium text-amber-600 mt-0.5">{{ __('events.events_as_coach') }}</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            @if($this->historyCount === 0)
                ...
                <div class="text-center py-12 sm:py-16 px-4">
                    <div class="relative inline-flex items-center justify-center w-20 h-20 sm:w-24 sm:h-24 rounded-full bg-gradient-to-br from-gray-100 to-gray-200 mb-6">
                        <x-heroicon-o-academic-cap class="w-10 h-10 sm:w-12 sm:h-12 text-gray-400" />
                    ...
                    <h3 class="text-lg sm:text-xl font-semibold text-gray-900">{{ __('events.no_coach_history') }}</h3>
                    <p class="mt-2 text-sm sm:text-base text-gray-500 max-w-md mx-auto">{{ __('events.no_coach_history_desc') }}</p>
                </div>
            @else
                {{ $this->table }}
            @endif
