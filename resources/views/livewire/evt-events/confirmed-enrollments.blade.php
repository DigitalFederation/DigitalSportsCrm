@php
    $namespace = $this->isFederation ? 'federation' : 'entity';
    $wizardSteps = [
...
<div class="space-y-6" x-data="{ loaded: false }" x-init="setTimeout(() => loaded = true, 100)">
        ...
        <div class="px-6 py-6 sm:px-8">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div class="flex-1">
                    ...
                    <p class="mt-1 text-gray-500 text-sm">{{ __('events.confirmed_enrollments_info') }}</p>
            ...
            @if($event->isRegistrationOpen())
                <div class="mt-5 pt-4 border-t border-gray-100 flex items-center gap-4">
                    ...
                    <a href="{{ $this->step2Route }}"
                       class="inline-flex items-center gap-2 px-4 py-2 bg-white border border-primary-light rounded-lg font-medium text-sm text-primary tracking-wide shadow-sm hover:bg-secondary-light focus:outline-none focus:border-primary focus:ring focus:ring-primary-light/30 transition-colors duration-150">
                        <x-heroicon-m-arrow-left class="w-4 h-4" />
                        {{ __('events.back_to_step2') }}
                    </a>
                </div>
            @endif
        </div>
    ...
    {{-- Summary Cards Grid --}}
    <div class="grid grid-cols-2 lg:grid-cols-{{ $this->isFederation ? '4' : '3' }} gap-3 sm:gap-4"
         x-show="loaded"
         x-transition:enter="transition ease-out duration-500 delay-100"
         x-transition:enter-start="opacity-0 transform translate-y-4"
         x-transition:enter-end="opacity-100 transform translate-y-0">

        @foreach(['athletes', 'coaches', 'officials'] as $role)
            @php
                ...
                @if($isActive)
                    <div class="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2">
                        <span class="flex h-3 w-3">
                            <span class="animate-ping absolute inline-flex h-full w-full rounded-full {{ str_replace('bg-', 'bg-', $config['iconBg']) }} opacity-75"></span>
        ...
        @if ($this->isFederation)
            @php
                $config = $roleConfig['referees'];
                $count = $this->refereesCount;
                $isActive = $activeTab === 'referees';
            @endphp
            <button wire:click="setActiveTab('referees')"
                    wire:loading.attr="disabled"
                    class="relative group text-left bg-white rounded-xl border-2 p-4 sm:p-5 transition-all duration-300 ease-out focus:outline-none focus:ring-2 focus:ring-offset-2 {{ $config['ring'] }}
                           {{ $isActive ? $config['border'] . ' ' . $config['bg'] . ' shadow-md ring-2 ' . $config['ring'] : 'border-gray-100 hover:border-gray-200 hover:shadow-md' }}"
                    aria-pressed="{{ $isActive ? 'true' : 'false' }}"
                    aria-label="{{ __('events.referees_tab') }}">
                ...
                <div class="flex items-start justify-between">
                    <div class="flex-1 min-w-0">
                        <p class="text-xs sm:text-sm font-medium text-gray-500 truncate">{{ __('events.referees_tab') }}</p>
                        <p class="mt-1 text-2xl sm:text-3xl font-bold text-gray-900 tabular-nums transition-transform duration-200 {{ $isActive ? 'scale-105' : 'group-hover:scale-105' }}">
                            {{ $count }}
                        </p>
                        @if($count > 0)
                    ...
                    <div class="flex-shrink-0 p-2.5 sm:p-3 rounded-xl {{ $config['iconBg'] }} transition-transform duration-200 {{ $isActive ? 'scale-110' : 'group-hover:scale-110' }}">
                        <x-heroicon-o-scale class="w-5 h-5 sm:w-6 sm:h-6 {{ $config['iconColor'] }}" />
                    </div>
                </div>

                <div wire:loading wire:target="setActiveTab('referees')"
                     class="absolute inset-0 bg-white/50 backdrop-blur-sm rounded-xl flex items-center justify-center">
                    <svg class="animate-spin h-5 w-5 {{ $config['iconColor'] }}" fill="none" viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            ...
            <nav class="flex overflow-x-auto scrollbar-hide -mb-px" aria-label="Tabs">
                ...
                @if ($this->isFederation)
                    ...
                    <button wire:click="setActiveTab('referees')"
                            ...
                            aria-selected="{{ $isActive ? 'true' : 'false' }}">
                        <span class="flex items-center gap-2">
                            <x-heroicon-o-scale class="w-4 h-4 sm:w-5 sm:h-5" />
                            <span class="hidden sm:inline">{{ __('events.referees_tab') }}</span>
                            <span class="inline-flex items-center justify-center min-w-[1.5rem] px-1.5 py-0.5 rounded-full text-xs font-semibold transition-colors
                        ...
                        @if($isActive)
                            <span class="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-purple-500 to-purple-600"></span>
                        @endif
                    </button>
                @endif
            </nav>
                ...
                <div class="text-center py-12 sm:py-16 px-4">
                    ...
                    @if($event->isRegistrationOpen())
                        <div class="mt-8 flex flex-col sm:flex-row items-center justify-center gap-3">
                            <a href="{{ $this->step1Route }}"
                               class="inline-flex items-center justify-center w-full sm:w-auto px-5 py-2.5 text-sm font-medium text-primary bg-white border border-primary-light rounded-lg shadow-sm hover:bg-secondary-light focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-all duration-200">
                                <x-heroicon-m-arrow-left class="w-4 h-4 mr-2" />
                                {{ __('events.back_to_step1') }}
                            </a>
                            <a href="{{ $this->step2Route }}"
                               class="inline-flex items-center justify-center w-full sm:w-auto px-5 py-2.5 text-sm font-medium text-white bg-primary rounded-lg shadow-sm hover:bg-primary-light focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-all duration-200">
                                <x-heroicon-m-arrow-left class="w-4 h-4 mr-2" />
                                {{ __('events.back_to_step2') }}
                            </a>
                        </div>
            ...
            <div class="px-4 sm:px-6 py-3 bg-white/50 border-t border-gray-200/80">
                <div class="flex flex-wrap items-center justify-center sm:justify-start gap-x-6 gap-y-2 text-xs text-gray-500">
                    <span class="inline-flex items-center gap-1.5">
                        ...
                        {{ $this->athletesCount }} {{ __('events.athletes_tab') }}
                    </span>
                    <span class="inline-flex items-center gap-1.5">
                        <span class="w-2 h-2 rounded-full bg-emerald-500"></span>
                        {{ $this->coachesCount }} {{ __('events.coaches_tab') }}
                    </span>
                    <span class="inline-flex items-center gap-1.5">
                        <span class="w-2 h-2 rounded-full bg-amber-500"></span>
                        {{ $this->officialsCount }} {{ __('events.officials_tab') }}
                    </span>
                    @if ($this->isFederation)
                        <span class="inline-flex items-center gap-1.5">
                            <span class="w-2 h-2 rounded-full bg-purple-500"></span>
                            {{ $this->refereesCount }} {{ __('events.referees_tab') }}
                        </span>
                    @endif
                </div>
            </div>
