<div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div>
            @if($selectedCommittee)
                <select class="form-input w-full" wire:model.live="selectedCertificationType">
                    <option value="">{{ __('admin.role_mappings.all_certification_types') }}</option>
                    @foreach($certificationTypes as $key => $type)
        ...
        <div class="flex justify-end">
            @if(count($selectedBulkCertifications) > 0)
                <button class="btn btn-primary" wire:click="openBulkModal">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                    </svg>
                    {{ __('admin.role_mappings.bulk_edit_roles', ['count' => count($selectedBulkCertifications)]) }}
                </button>
            @endif
        </div>
    </div>
    ...
    <!-- Table -->
    <x-dynamic-table
        :headers="['', __('admin.role_mappings.certification_name'), __('admin.role_mappings.acronym'), __('admin.role_mappings.certification_type'), __('admin.role_mappings.committee'), __('admin.role_mappings.professional_role'), __('admin.role_mappings.assigned_roles'), __('admin.role_mappings.actions')]">
            ...
            <tr>
                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap w-px">
                    <input type="checkbox" 
                           ...
                           wire:model.live="selectedBulkCertifications">
                </td>
                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">{{ $certification->name }}</td>
                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">{{ $certification->acronym }}</td>
                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">{{ $certification->certification_category ?? '-' }}</td>
                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">{{ $certification->committee->name ?? '-' }}</td>
                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                    @if($certification->professionalRole)
                        <span class="px-2 py-1 bg-gray-200 text-gray-800 text-xs font-medium rounded-full">{{ $certification->professionalRole->name }}</span>
                    @else
                        -
                    @endif
                </td>
                <td class="px-2 first:pl-5 last:pr-5 py-3">
                    @if(isset($certificationRoles[$certification->id]))
                        <div class="flex flex-wrap gap-1">
                            @foreach($certificationRoles[$certification->id] as $role)
                ...
                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap w-px">
                    <button class="btn btn-sm btn-outline-primary" 
                            wire:click="editRoles({{ $certification->id }})">
        ...
        @empty
            <tr>
                <td colspan="8" class="px-2 first:pl-5 last:pr-5 py-8 text-center text-slate-500">
                    {{ __('admin.role_mappings.no_certifications_found') }}
                </td>
            </tr>
        @endforelse
    </x-dynamic-table>
            ...
            <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white" wire:click.stop>
                <div class="mt-3">
                    ...
                    <div class="flex items-center justify-between pb-4 border-b border-gray-200">
                        <h3 class="text-lg font-semibold text-gray-900">{{ __('admin.role_mappings.edit_certification_roles') }}</h3>
        ...
        <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50" wire:click="closeBulkModal">
            <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white" wire:click.stop>
                <div class="mt-3">
                    <!-- Modal Header -->
                    <div class="flex items-center justify-between pb-4 border-b border-gray-200">
                        <h3 class="text-lg font-semibold text-gray-900">{{ __('admin.role_mappings.bulk_edit_title', ['count' => count($selectedBulkCertifications)]) }}</h3>
