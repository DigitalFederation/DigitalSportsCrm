    ...
    <div class="min-h-96">
        @if($currentStep === 'upload')
            ...
            <div class="text-center">
                ...
                <p class="text-sm text-slate-600 mb-6">{{ __('import.choose_file_description') }}</p>
        ...
        @elseif($currentStep === 'validation')
            ...
            <div>
                <div class="text-center mb-6">
                    ...
                    <p class="text-sm text-gray-600">Review your data before importing</p>
                        ...
                        <div class="bg-green-50 border border-green-200 rounded-lg p-4">
                            @foreach($validationResults['sample_valid_records'] as $record)
                                <div class="text-sm text-green-700 mb-1">
                                    Row {{ $record['row_number'] }}: {{ $record['data']['name'] ?? '' }} {{ $record['data']['surname'] ?? '' }}, {{ $record['data']['email'] ?? '' }} ✓
                                </div>
                            @endforeach
                        </div>
                    </div>
                ...
                @if(!empty($validationErrors))
                    <div class="mb-6">
                        <h4 class="text-sm font-medium text-slate-800 mb-2">{{ __('import.errors') }}</h4>
                        <div class="bg-red-50 border border-red-200 rounded-lg p-4 max-h-40 overflow-y-auto">
                            @foreach($validationErrors as $rowNumber => $errors)
                                @if(is_array($errors))
                                    <div class="mb-2">
                                        <div class="text-sm font-medium text-red-800">Row {{ $rowNumber }}:</div>
                                        @foreach($errors as $error)
                                            @if(is_string($error))
                                                <div class="text-sm text-red-700 ml-4">• {{ $error }}</div>
                                            @elseif(is_array($error))
                                                @foreach($error as $subError)
                                                    <div class="text-sm text-red-700 ml-4">• {{ is_string($subError) ? $subError : json_encode($subError) }}</div>
                                                @endforeach
                                            @endif
                                        @endforeach
                                    </div>
                                @elseif(is_string($errors))
                                    <div class="text-sm text-red-700 mb-1">❌ {{ $errors }}</div>
                                @endif
                            @endforeach
                        </div>
                    </div>
                ...
                @if(!empty($validationWarnings))
                    <div class="mb-6">
                        <h4 class="text-sm font-medium text-slate-800 mb-2">{{ __('import.warnings') }}</h4>
                        <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 max-h-40 overflow-y-auto">
                            @foreach($validationWarnings as $rowNumber => $warnings)
                                @if(is_array($warnings))
                                    <div class="mb-2">
                                        <div class="text-sm font-medium text-yellow-800">Row {{ $rowNumber }}:</div>
                                        @foreach($warnings as $warning)
                                            @if(is_string($warning))
                                                <div class="text-sm text-yellow-700 ml-4">⚠ {{ $warning }}</div>
                                            @elseif(is_array($warning))
                                                @foreach($warning as $subWarning)
                                                    <div class="text-sm text-yellow-700 ml-4">⚠ {{ is_string($subWarning) ? $subWarning : json_encode($subWarning) }}</div>
                                                @endforeach
                                            @endif
                                        @endforeach
                                    </div>
                                @elseif(is_string($warnings))
                                    <div class="text-sm text-yellow-700 mb-1">⚠ {{ $warnings }}</div>
                                @endif
                            @endforeach
                        </div>
                    </div>
                    ...
                    @if($showAdvancedOptions)
                        <div class="mt-4 space-y-4">
                            <div>
                                ...
                                <p class="text-xs text-slate-500 mb-2">{{ __('import.federations_help') }}</p>
                            ...
                            <div class="flex items-center">
                                <input wire:model="sendNotifications" type="checkbox" class="form-checkbox">
                                <label class="ml-2 block text-sm text-slate-800">{{ __('import.send_welcome_notifications') }}</label>
                            </div>

                            <div class="flex items-center">
                                <input wire:model="autoAssignFederation" type="checkbox" class="form-checkbox">
                                <label class="ml-2 block text-sm text-slate-800">{{ __('import.auto_assign_federation') }}</label>
                            </div>
                        </div>
                    @endif
                </div>
        ...
        @elseif($currentStep === 'import')
            ...
            <div class="text-center">
                <div class="mx-auto h-24 w-24 mb-4 {{ $importStatus === 'failed' ? 'text-red-600' : 'text-blue-600' }}">
                    @if($importInProgress)
                ...
                @if($importInProgress)
                    ...
                    <div class="max-w-md mx-auto">
                        ...
                        @if(!empty($importResults))
                            ...
                            @if(isset($importResults['estimated_time_remaining']))
                                <p class="mt-4 text-sm text-slate-600">{{ __('import.estimated_time_remaining', ['time' => $importResults['estimated_time_remaining']]) }}</p>
                            @endif
                        @endif
                ...
                @elseif($importStatus === 'failed' || isset($errors['import']))
                    <h3 class="text-lg font-medium text-red-800 mb-2">{{ __('import.import_failed') }}</h3>
                    <p class="text-sm text-red-600 mb-4">{{ __('import.import_failed_message') }}</p>

                    @if($importErrorMessage || isset($errors['import']))
                        <div class="max-w-2xl mx-auto mb-6">
                            <div class="bg-red-50 border border-red-200 rounded-lg p-4">
                                <h4 class="text-sm font-medium text-red-800 mb-2">{{ __('import.error_details') }}:</h4>
                                <div class="max-h-48 overflow-y-auto">
                                    <pre class="text-xs text-red-700 whitespace-pre-wrap break-words font-mono">{{ $importErrorMessage ?? $errors['import'] ?? '' }}</pre>
                                </div>
                            </div>
                        </div>
                    @endif
