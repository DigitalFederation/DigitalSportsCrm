                        ...
                        <div class="mb-4">
                            <div class="flex items-start justify-between">
                                ...
                                @if($template->hasEntityApplied)
                                    @include('web.entity.event_applications.components.already-applied-badge')
                                @else
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                        {{ __('event_applications.badges.open') }}
                                    </span>
                                @endif
                            </div>
                            ...
                            <div class="mt-1 flex items-center gap-2 text-sm text-slate-600">
                                ...
                                @if($template->sport)
                                    <span>•</span>
                                    <span>{{ $template->sport->translated_name }}</span>
                                @endif
                            </div>
                        </div>
                                ...
                                <div class="space-y-2">
                                    ...
                                    <a href="{{ route('entity.event-applications.show', $template->existingApplication) }}"
                                       class="btn btn-secondary w-full justify-center">
                                        ...
                                        <span class="ml-2">{{ __('event_applications.actions.view_application') }}</span>
                                    </a>
                                </div>
                            @else
                                <a href="{{ route('entity.event-applications.create-from-template', $template) }}"
                                   class="btn btn-primary w-full justify-center">
                                    ...
                                    <span class="ml-2">{{ __('event_applications.actions.apply_now') }}</span>
                                </a>
                            @endif
                        </div>
                    </div>
