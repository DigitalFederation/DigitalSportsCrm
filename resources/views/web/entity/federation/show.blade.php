<x-layout>
    @php
        ...
        $statusName = $entityFederation?->stateName() ?? 'Unknown';
        ...
        $statusColor = strtolower($entityFederation?->stateColor() ?? 'gray');
    @endphp

    <div class="previous-layout-classes">
        <!-- Back Button -->
        <div class="mb-4">
            <a href="{{ route('entity.federation.index') }}"
               class="btn btn-sm btn-secondary">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="w-4 h-4">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" />
                </svg>
                <span class="ml-2">{{ __('main.Back') }}</span>
            </a>
        </div>
        ...
        <div class="mb-6">
            <div class="flex items-center gap-3">
                <h1 class="text-2xl font-bold text-slate-800">{{ $federation->legal_name }}</h1>
                <x-tables.badge :status="$translatedStatus" :color="$statusColor" />
            </div>
            <p class="text-sm text-slate-500 mt-1">{{ $federation->is_local ? __('National Organization') : __('National Federation') }}</p>
        </div>
            ...
            <div class="bg-white border border-slate-200 rounded-lg p-5">
                ...
                <dl class="space-y-4">
                    ...
                    <div>
                        ...
                        <dd class="mt-1 text-sm font-semibold text-slate-800">{{ $federation->legal_name }}</dd>
                    </div>
