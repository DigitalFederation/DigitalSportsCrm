...
<x-layout>
    <div x-data="{ showInviteModal: false, pendingCount: {{ $pendingInvitations->count() }} }"
         @diving-professional-invited.window="showInviteModal = false; setTimeout(() => window.location.reload(), 500)">
        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-6">
            ...
            <div class="mb-4 sm:mb-0">
                <div class="flex items-center gap-3">
                    <h1 class="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">{{ __('diving.diving_professionals') }}</h1>
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">
                    {{ __('diving.associated_to') }} <span class="font-semibold text-slate-700 dark:text-slate-300">{{ $entity->name }}</span>
                </p>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                >
                    ...
                    <span>{{ __('diving.invite_professional') }}</span>
                </button>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-5">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-sm font-medium text-slate-500 dark:text-slate-400">{{ __('diving.total_professionals') }}</p>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-5">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-sm font-medium text-slate-500 dark:text-slate-400">{{ __('diving.active_professionals') }}</p>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-5">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-sm font-medium text-slate-500 dark:text-slate-400">{{ __('diving.pending_invitations') }}</p>
            ...
            <div class="border-b border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50">
                <nav class="flex gap-x-1 px-4" aria-label="Tabs">
                    ...
                    >
                        ...
                        <span>{{ __('diving.associated_professionals') }}</span>
                        @if($instructors->total() > 0)
                            <span
                    ...
                    >
                        ...
                        <span>{{ __('diving.pending_invitations_tab') }}</span>
                        <span
                ...
                <div x-show="activeTab === 'active'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-cloak>
                                    ...
                                    @foreach($instructors as $instructor)
                                        @php
                                            ...
                                            $colorClasses = match($statusClassBase) {
                                                ...
                                                'RejectedEntityProfessionalRoleState' => 'bg-rose-50 text-rose-700 dark:bg-rose-900/30 dark:text-rose-400',
                                            ...
                                            $systems = [];
                                            if ($instructor->individual->certificationsAttributed()->certificationAttributedStatus('active')->exists()) {
                                                $systems[] = 'CMAS';
                                            }
                                            $otherSystems = $instructor->individual->divingProfessionalCertifications()
                                                ->active()
                                                ->pluck('certification_system')
                                                ->unique()
                                                ->toArray();
                                            $systems = array_unique(array_merge($systems, $otherSystems));
                                        @endphp
                                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                            <td class="px-5 py-4 whitespace-nowrap">
                                                <div class="flex items-center gap-3">
                                                    <img class="flex-shrink-0 w-9 h-9 rounded-lg object-cover shadow-sm"
                                                         src="{{ $instructor->individual?->avatar_url }}"
                                                         alt="{{ $instructor->individual?->full_name }}" />
                                                    <div>
                                                        <div class="text-sm font-medium text-slate-900 dark:text-white">
                                                            {{ $instructor->individual?->name }} {{ $instructor->individual?->surname }}
                                                        </div>
                                                        <div class="text-xs text-slate-500 dark:text-slate-400">
                                                            {{ $instructor->individual?->email }}
                                                        </div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td class="px-5 py-4 whitespace-nowrap">
                                                <span class="text-sm font-mono text-slate-700 dark:text-slate-300">
                                                    {{ $instructor->individual?->member_number ?? '-' }}
                                                </span>
                                            </td>
                    ...
                    @else
                        ...
                        <div class="text-center py-12">
                            ...
                            <h3 class="text-base font-semibold text-slate-900 dark:text-white mb-1">{{ __('diving.no_professionals') }}</h3>
                            <p class="text-sm text-slate-500 dark:text-slate-400">{{ __('diving.start_by_inviting_professional') }}</p>
                            <div class="mt-4">
                                <button @click="showInviteModal = true" class="btn btn-primary">
                                    {{ __('diving.invite_first_professional') }}
                                </button>
                            </div>
                        </div>
                    @endif
                ...
                <div x-show="activeTab === 'pending'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-cloak>
                                ...
                                <tbody class="bg-white dark:bg-slate-800 divide-y divide-slate-200 dark:divide-slate-600">
                                    @foreach($pendingInvitations as $invitation)
                                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                                ...
                                                <div class="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div class="text-xs text-slate-500 dark:text-slate-400 font-mono">
                                                            {{ $invitation->individual?->member_number }}
                                                        </div>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td class="px-5 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <form action="{{ route('entity.diving_professionals.cancel_invitation', $invitation) }}" method="POST" class="inline" onsubmit="return confirm('{{ __('common.confirm_delete') }}');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="p-1.5 rounded-lg text-slate-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 transition-colors" title="{{ __('diving.cancel') }}">
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @else
                        ...
                        <div class="text-center py-12">
                            ...
                            <h3 class="text-base font-semibold text-slate-900 dark:text-white mb-1">{{ __('diving.no_pending_invitations') }}</h3>
                            <p class="text-sm text-slate-500 dark:text-slate-400">{{ __('diving.no_pending_invitations_desc') }}</p>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="flex items-center justify-center min-h-screen px-4 sm:pl-72">
                ...
                <div x-show="showInviteModal"
                    ...
                    <div class="bg-slate-50 dark:bg-slate-800/50 px-6 py-4 border-b border-slate-200 dark:border-slate-700 rounded-t-xl">
                        <div class="flex items-center justify-between">
                            <h3 class="text-lg font-semibold text-slate-900 dark:text-white">
                                {{ __('diving.invite_new_professional') }}
                            </h3>
                    ...
                    <div class="px-6 py-4">
                            ...
                            <div class="flex">
                                ...
                                <div class="ml-3">
                                    <p class="text-sm text-blue-700 dark:text-blue-300">
                                        <strong>{{ __('diving.conditions') }}:</strong>
                                        {{ __('diving.condition_diving_professional_certification') }}
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <livewire:manage-entity-diving-professionals :professionalRoles="$professionalRoles" :showInviteSection="true" />
                    </div>
                </div>
            </div>
        </div>
    ...
    <!-- Include deactivation modals for each professional -->
    @foreach ($instructors as $instructor)
        <x-professional-deactivation-modal
            ...
            :action="route('entity.diving_professionals.remove', $instructor->id)" />
    @endforeach
</x-layout>
