              ...
              <div class="bg-white shadow-lg rounded-lg relative flex flex-col min-w-0 break-words w-full mb-6">
                    ...
                    <table class=" w-full mb-4 text-gray-800 border-collapse">
                        ...
                        @foreach($document->details as $detail)
                          <tr>
                              ...
                              <td class="py-4 p-3 text-right ">{{ money($detail->unit_value) }}</td>
                              <td class="py-4 p-3 text-right ">{{ money($detail->total_value) }}</td>
                          </tr>
                        @endforeach
                        </tbody>
                        <tfoot>
                          <tr class="mt-4">
                              ...
                              <th class="border-b-0 p-3 border-t" colspan="3">
                                <p class="text-right text-lg font-semibold pt-2"> {{ money($document->total_value) }}</p>
                              </th>
                          </tr>
                        </tfoot>
                    </table>
                  ...
                  <div class="px-4 py-3 border-t">
                    <div class="text-left ml-auto px-4 relative w-full">
                        @php($internationalName = config('branding.international.name', 'the International Federation'))
                        <p class="italic text-sm">Federations and International Organisations hereby undertake to comply with and strictly
                          enforce {{ $internationalName }} rules, as well as to urge their members to adopt an under water environmental
                          friendly attitude.</p>
                        <p class="text-sm mt-4 border-t border-gray-400 pt-4 italic">
                          ...
                          faire appliquer strictement les règles de {{ $internationalName }} et à inciter leurs membres à adopter une
