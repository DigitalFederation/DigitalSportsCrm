...
<x-layout>
    <div class="previous-layout-classes">
        ...
        <!-- FILTER RESULTS -->
        <x-filter-form :post="route('individual.document.index')">
            <x-forms.filter-input-select :label="__('documents.filters.category')" name="owner_type" :options="$filter_owner_types" />
        ...
        <div class="sm:flex sm:justify-center sm:items-center mb-5 mt-8">
            ...
            <x-dynamic-table
                :headers="[__('documents.table.number'), __('documents.table.date'), __('documents.table.type'), __('documents.table.status'), ['text'=>__('documents.table.total'), 'alignment'=>'text-right'], '']">
                            ...
                            <div class="space-x-1 flex justify-end items-end">
                                @if($document->moloniInvoice)
                                    <x-dynamic-table-buttons type="document.moloni"
                                                             :route="route('individual.document.moloni-pdf', $document->id)"
                                                             target="_blank" />
                                @endif
                                <x-dynamic-table-buttons type="document.pdf"
                                                         :route="route('individual.document.download', $document->id)"
                                                         target="_blank" />
                                <x-dynamic-table-buttons type="show"
                                                         :route="route('individual.document.show', $document->id)" />
