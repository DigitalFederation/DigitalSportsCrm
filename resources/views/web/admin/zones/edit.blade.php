<x-layout>
    <div class="previous-layout-classes">
        ...
        <div class="mb-8 flex justify-between">
            <h1 class="page-first-title">{{ __('Edit Zone') }}: {{ $zone->name }}</h1>
        </div>
                        ...
                        <div class="flex flex-wrap -mx-4 space-y-4 md:space-y-0">
                            ...
                            <div class="w-full px-4 md:w-1/2">
                                ...
                                <input id="code" class="form-input w-full" type="text" 
                                       name="code" value="{{ old('code', $zone->code) }}" 
                            ...
                            <div class="w-full px-4 md:w-1/2">
                                ...
                                <select id="is_active" class="form-input w-full" name="is_active">
                                    <option value="1" {{ old('is_active', $zone->is_active) == '1' ? 'selected' : '' }}>
                                        {{ __('Active') }}
                                    </option>
                                    <option value="0" {{ old('is_active', $zone->is_active) == '0' ? 'selected' : '' }}>
                                        {{ __('Inactive') }}
                                    </option>
                                </select>
                                @error('is_active')
                            ...
                            <div class="w-full px-4 md:w-1/2">
                                ...
                                <textarea id="description" class="form-input w-full" name="description" rows="3" 
                                          placeholder="{{ __('Optional description of the zone') }}">{{ old('description', $zone->description) }}</textarea>
                                @error('description')
                                    <div class="text-xs mt-1 text-rose-500">{{ $message }}</div>
                                @enderror
                            </div>
                                                ...
                                                @foreach($countryDistricts as $district)
                                                    <label class="flex items-center space-x-3 p-2 bg-slate-50 rounded hover:bg-slate-100 cursor-pointer">
                                                        <input type="checkbox" name="district_ids[]" value="{{ $district->id }}" 
                                                               class="form-checkbox" 
                                                               {{ in_array($district->id, old('district_ids', $selectedDistrictIds)) ? 'checked' : '' }}>
                                                        <div>
            ...
            countryGroups.forEach(group => {
                ...
                if (header && checkboxes.length > 1) {
                    ...
                    selectAllBtn.addEventListener('click', function() {
                        ...
                        checkboxes.forEach(cb => cb.checked = !currentlyAllChecked);
                        selectAllBtn.textContent = currentlyAllChecked ? 'Select All' : 'Deselect All';
                    });
