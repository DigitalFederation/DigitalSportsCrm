<x-layout>
    <div class="previous-layout-classes">
        ...
        <div class="mb-8 flex justify-between">
            <h1 class="page-first-title">{{ __('Edit District') }}: {{ $district->name }}</h1>
        </div>
                        ...
                        <div class="flex flex-wrap -mx-4 space-y-4 md:space-y-0">
                            ...
                            <div class="w-full px-4 md:w-1/2">
                                ...
                                <select id="is_active" class="form-input w-full" name="is_active">
                                    <option value="1" {{ old('is_active', $district->is_active) == '1' ? 'selected' : '' }}>
                                        {{ __('Active') }}
                                    </option>
                                    <option value="0" {{ old('is_active', $district->is_active) == '0' ? 'selected' : '' }}>
                                        {{ __('Inactive') }}
                                    </option>
                                </select>
                                @error('is_active')
                            ...
                            <div class="w-full px-4">
                                ...
                                <textarea id="description" class="form-input w-full" name="description" rows="3" 
                                          placeholder="{{ __('Optional description of the district') }}">{{ old('description', $district->description) }}</textarea>
                                @error('description')
                                    <div class="text-xs mt-1 text-rose-500">{{ $message }}</div>
                                @enderror
                            </div>
