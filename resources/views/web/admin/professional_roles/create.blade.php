<x-layout>
    <div class="previous-layout-classes">
        ...
        <div class="mb-8 flex justify-between">
            <h1 class="page-first-title">{{ __('professional_roles.create') }}</h1>
        </div>

        <form action="{{ route('admin.professional-roles.store') }}" method="POST">
            @csrf
                ...
                <div class="flex flex-col md:flex-row md:-mr-px">
                    <div class="grow">
                        <section class="p-6 space-y-6">
                            <!-- Information Box -->
                            <x-information-box
                                :title="__('professional_roles.info_title')"
                                :body="__('professional_roles.info_body')"
                            />
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <input id="name" class="form-input w-full @error('name') border-rose-500 @enderror" type="text" name="name" value="{{ old('name') }}" required />
                                    @error('name')
                                ...
                                <div>
                                    ...
                                    <input id="code" class="form-input w-full @error('code') border-rose-500 @enderror" type="text" name="code" value="{{ old('code') }}" required placeholder="FINSWIMMINGCOACH" />
                                    @error('code')
                                        <div class="text-xs mt-1 text-rose-500">{{ $message }}</div>
                                    @enderror
                                ...
                                <div>
                                    ...
                                    <select id="role" class="form-select w-full @error('role') border-rose-500 @enderror" name="role" required>
                                        ...
                                        @foreach($roleTypes as $roleType)
                                            <option value="{{ $roleType }}" {{ old('role') === $roleType ? 'selected' : '' }}>{{ __('professional_roles.role_types.' . $roleType) }}</option>
                                        @endforeach
                                    </select>
                                    @error('role')
                                        <div class="text-xs mt-1 text-rose-500">{{ $message }}</div>
                                ...
                                <div>
                                    ...
                                    <select id="committee_id" class="form-select w-full @error('committee_id') border-rose-500 @enderror" name="committee_id">
                                        ...
                                        @foreach($committees as $committee)
                                            <option value="{{ $committee->id }}" {{ old('committee_id') == $committee->id ? 'selected' : '' }}>{{ $committee->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('committee_id')
                                        <div class="text-xs mt-1 text-rose-500">{{ $message }}</div>
                        ...
                        <section>
                            <div class="flex flex-col px-6 py-5 border-t border-slate-200 dark:border-slate-700">
                                <div class="flex self-end">
                                    ...
                                    <button type="submit" class="btn btn-primary ml-3">{{ __('professional_roles.create') }}</button>
                                </div>
                            </div>
                        </section>
                    </div>
