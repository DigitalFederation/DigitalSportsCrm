@section('title', __('events.technical_officials'))
        ...
        @php
            $actions = [
                ...
                [
                    ...
                    'url' => route('admin.evt-events.events.referee-enrollment.create', $event->id),
                    'text' => __('events.enroll_referee'),
                ],
            ];
            ...
            if (!empty($enrollments) && count($enrollments) > 0) {
                $actions[] = [
                    'type' => 'link',
                    'class' => 'btn btn-info',
                    'url' => route('admin.evt-events.events.referee-enrollment.export', $event->id),
                    'text' => __('events.export_referee_enrollments'),
                ];
            }
        @endphp

        <x-layout.page-header
            :title="__('events.technical_officials')"
            :subtitle="$event->name"
            :actions="$actions"
        />

        <x-filter-form :post="route('admin.evt-events.events.referee-enrollment.index', $event->id)">
                        ...
                        <tr>
                            ...
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap w-px !bg-slate-50">
                                <div class="flex flex-row items-center gap-2">
                                    <form
                                        action="{{ route('admin.evt-events.events.referee-enrollment.destroy', ['event' => $event->id, 'referee_enrollment' => $enrollment->id]) }}"
                                        method="POST" x-data="{}"
                                        @submit.prevent="if (confirm('{{ __('events.confirm_remove_referee') }}')) $el.submit();">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            class="text-red-500 hover:text-red-700 transition-colors duration-150 p-1 rounded-full hover:bg-red-50"
                                            title="{{ __('events.remove_referee') }}">
