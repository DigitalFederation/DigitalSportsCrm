...
<x-layout>
    <style>
        ...
        /* Ensure the last column is on top */
        table th:nth-last-child(1),
        table tbody tr td:nth-last-child(1) {
            z-index: 10;
        }
        ...
        <x-layout.page-header
            ...
            :actions="[
                [
                    'type' => 'link',
                    'class' => 'btn btn-info',
                    'url' => route('admin.evt-events.events.enrollments.athlete.index', $event->id),
                    'text' => __('events.back_to_enrollments'),
                ],
            ]"
        />

        <x-evt_event.enrollment-navigation :event="$event" active="athlete.registered" />

        <x-filter-form :post="route('admin.evt-events.events.enrollments.athlete.registered', $event->id)">
                            ...
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap w-px !bg-slate-50">
                                <div class="flex flex-row items-center gap-2">
                                    ...
                                    @if($enrollment->status_class->isCanceled())
                                        <form
                                            action="{{ route('admin.evt-events.events.enrollments.athlete.force-delete', ['event' => $event->id, 'athleteEnrollment' => $enrollment->id]) }}"
                                            method="POST"
                                            class="flex"
                                            onsubmit="return confirm('{{ __('events.confirm_delete_enrollment') }}');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-red-500 hover:text-red-700" title="{{ __('events.delete_permanently') }}">
