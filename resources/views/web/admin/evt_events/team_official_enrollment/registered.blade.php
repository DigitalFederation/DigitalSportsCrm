@section('title', __('events.pending_team_officials'))
...
<x-layout>
    ...
    <div class="previous-layout-classes">
        <x-layout.page-header
            :title="__('events.pending_team_officials')"
            :subtitle="$event->name"
            :actions="[
                [
                    'type' => 'link',
                    'class' => 'btn btn-info',
                    'url' => route('admin.evt-events.events.officials-enrollment.index', $event->id),
                    'text' => __('events.back_to_enrollments'),
                ],
            ]"
        />

        <x-evt_event.enrollment-navigation :event="$event" active="official.registered" />

        <x-filter-form :post="route('admin.evt-events.events.officials-enrollment.registered', $event->id)">
                            ...
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap w-px !bg-slate-50">
                                <div class="flex flex-row items-center gap-2">
                                    @if ($enrollment->state->isCanceled())
                                        <form
                                            action="{{ route('admin.evt-events.events.officials-enrollment.force-delete', ['event' => $event, 'officials_enrollment' => $enrollment]) }}"
                                            method="POST" x-data="{}"
