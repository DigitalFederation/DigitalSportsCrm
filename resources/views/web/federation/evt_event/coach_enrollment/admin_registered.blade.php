        ...
        <x-layout.page-header
            ...
            :actions="[
                [
                    'type' => 'link',
                    'class' => 'btn btn-info',
                    'url' => route('federation.evt-events.events.coach-enrollment.index', $event->id),
                    'text' => __('events.back_to_enrollments'),
                ],
            ]"
        />

        <x-evt_event.enrollment-navigation :event="$event" active="coach.registered" :links="$navigationLinks" />

        <x-filter-form :post="route('federation.evt-events.events.coach-enrollment.registered', $event->id)">
                            ...
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap w-px !bg-slate-50">
                                <div class="flex flex-row items-center gap-2">
                                    @if (!$enrollment->state->isCanceled())
                                        <form
                                            action="{{ route('federation.evt-events.events.coach-enrollment.destroy', ['event' => $event, 'coach_enrollment' => $enrollment]) }}"
                                            method="POST" x-data="{}"
