<x-layout>
    <div class="space-y-6" x-data="{ loaded: false }" x-init="setTimeout(() => loaded = true, 100)">
                    ...
                    <div class="flex-1">
                        <div class="flex items-center gap-3 mb-3">
                            <div class="flex items-center justify-center w-10 h-10 rounded-full bg-indigo-100">
                                <x-heroicon-s-pencil-square class="w-6 h-6 text-indigo-600" />
                            </div>
                        ...
                        <h1 class="text-xl sm:text-2xl font-semibold text-gray-900">{{ __('event_applications.titles.edit_template') }}</h1>
                        <p class="mt-1 text-gray-500 text-sm">{{ __('event_applications.header.edit_template_subtitle') }}</p>
                    </div>
                    ...
                    @if($application_template->applications_count > 0)
                        <div class="flex-shrink-0">
                            <div class="inline-flex flex-col items-center justify-center px-5 py-3 rounded-xl bg-gray-50 border border-gray-200">
                                <span class="text-3xl sm:text-4xl font-bold text-gray-900 tabular-nums">{{ $application_template->applications_count }}</span>
                                <span class="text-xs font-medium text-gray-500 mt-0.5">{{ __('event_applications.labels.applications_received') }}</span>
                            </div>
                        </div>
                    @endif
                </div>
        ...
        <form method="POST" action="{{ route($routeNamespace . '.application-templates.update', $application_template) }}">
            @csrf
            @method('PUT')
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div class="sm:col-span-2">
                                ...
                                <input type="text"
                                       ...
                                       value="{{ old('name', $application_template->name) }}"
                            ...
                            <div>
                                ...
                                <select id="event_type"
                                    ...
                                    <option value="organization" {{ old('event_type', $application_template->event_type) === 'organization' ? 'selected' : '' }}>
                                        {{ __('event_applications.event_types.organization') }}
                                    </option>
                                    <option value="competition" {{ old('event_type', $application_template->event_type) === 'competition' ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select id="sport_id"
                                    ...
                                    @foreach($sports as $sport)
                                        <option value="{{ $sport->id }}" {{ old('sport_id', $application_template->sport_id) == $sport->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select id="registration_type"
                                    ...
                                    @foreach(['entities', 'entities_individuals', 'individuals', 'federations', 'federations_entities', 'federations_entities_individuals'] as $type)
                                        <option value="{{ $type }}" {{ old('registration_type', $application_template->registration_type) === $type ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select id="category"
                                    ...
                                    @foreach(['A', 'B', 'C', 'D'] as $cat)
                                        <option value="{{ $cat }}" {{ old('category', $application_template->category) === $cat ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <input type="text"
                                       ...
                                       value="{{ old('age_group', $application_template->age_group) }}">
                                @error('age_group')
                                    <div class="text-xs mt-1 text-rose-500">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <textarea id="description"
                                          ...
                                          placeholder="{{ __('event_applications.placeholders.template_name') }}">{{ old('description', $application_template->description) }}</textarea>
                                @error('description')
                                    <div class="text-xs mt-1 text-rose-500">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="date"
                                       ...
                                       value="{{ old('submission_start_date', $application_template->submission_start_date?->format('Y-m-d')) }}"
                                       required>
                            ...
                            <div>
                                ...
                                <input type="date"
                                       ...
                                       value="{{ old('submission_end_date', $application_template->submission_end_date?->format('Y-m-d')) }}"
                                       required>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="date"
                                       ...
                                       value="{{ old('event_start_date', $application_template->event_start_date?->format('Y-m-d')) }}">
                            ...
                            <div>
                                ...
                                <input type="date"
                                       ...
                                       value="{{ old('event_end_date', $application_template->event_end_date?->format('Y-m-d')) }}">
                            ...
                            <div>
                                ...
                                <input type="number"
                                       ...
                                       value="{{ old('max_applications', $application_template->max_applications) }}">
                                ...
                                @foreach(['both', 'entities', 'federations'] as $audience)
                                    <label class="flex items-center">
                                        <input type="radio"
                                               ...
                                               value="{{ $audience }}"
                                               class="form-radio"
                                               {{ old('target_audience', $application_template->target_audience) === $audience ? 'checked' : '' }}>
                            ...
                            <div class="mt-4">
                                <div class="information-box">
                                    <p class="text-sm text-slate-600">
                                        {{ __('event_applications.messages.applications_count', ['count' => $application_template->applications_count]) }}
                                    </p>
                                </div>
                            </div>
                        @endif
