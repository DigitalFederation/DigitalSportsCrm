@section('title', __('federation.create_entity_affiliation'))
<x-layout>
    <div class="previous-layout-classes">
        <!-- Page Header -->
        <div class="sm:flex sm:justify-between sm:items-center mb-5 mt-5">
            <div class="mb-4 sm:mb-0">
                <h1 class="page-first-title">{{ __('federation.add_entity_affiliation') }}</h1>
                <p class="text-gray-500 text-sm">{{ __('federation.entity_affiliations_section.create_subtitle') }}</p>
            </div>
        ...
        <div class="information-box mb-8">
            <div class="flex items-start">
                ...
                <div class="text-sm">
                    ...
                    <p class="text-slate-600 mt-1">{{ __('federation.entity_affiliation_notice') }}</p>
                </div>
            </div>
        </div>
        ...
        @if(isset($availablePackages) && $availablePackages->count() > 0)
            ...
            <div class="card mb-8">
                <div class="sm:flex sm:justify-between sm:items-center mb-5">
                    <h2 class="text-xl leading-snug text-slate-800 font-bold">{{ __('federation.entity_affiliations_section.select_membership_package') }}</h2>
                </div>
                        ...
                        <div class="card">
                            <div class="flex justify-between items-start mb-3">
                                ...
                                <span class="text-lg font-bold text-indigo-600">
                                    €{{ number_format($package->calculatePriceFor(\Domain\Entities\Models\Entity::class), 2) }}
                                </span>
                            </div>
                            ...
                            <div class="space-y-2 mb-4">
                                ...
                                @if($package->insurancePlans->count() > 0)
                                    <div class="text-xs">
                                        ...
                                        <span class="text-slate-600">{{ __('common.yes') }}</span>
                                    </div>
                                @endif
                            </div>
            ...
            <div class="card" 
                ...
                <div x-show="!showForm" class="text-center py-12">
                    <div class="text-slate-500">{{ __('federation.entity_affiliations_section.select_package_to_continue') }}</div>
                </div>

                <div x-show="showForm" x-cloak>
                    <div class="sm:flex sm:justify-between sm:items-center mb-5">
                        <h2 class="text-xl leading-snug text-slate-800 font-bold">{{ __('federation.create_entity_affiliation') }}</h2>
                    </div>

                    <form method="POST" action="{{ route('federation.entity-affiliations.store') }}">
                        @csrf
                        ...
                        <div class="flex flex-wrap justify-end space-x-2 mt-6">
                            ...
                            <button type="submit" class="btn btn-primary">
                                {{ __('federation.create_affiliation') }}
                            </button>
                        </div>
                    </form>
                </div>
        ...
        @else
            ...
            <div class="card">
                <div class="text-center py-12">
                    <div class="text-slate-500 mb-4">{{ __('federation.no_packages_available_federation_distribution') }}</div>
                    <a href="{{ route('federation.entity-affiliations.index') }}" class="btn btn-secondary">
                        {{ __('common.back') }}
                    </a>
                </div>
